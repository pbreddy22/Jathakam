...
public class Extlib implements java.io.Serializable {
  ...
  ISwissEph swissEph=new SwissEph(); 
  SwissLib  swissLib=new SwissLib();
  SweDate   sweDate1=new SweDate();
  SweDate   sweDate2=new SweDate();
  SweDate   sdu1, sdu2;
  SwissData swissData=new SwissData();
...
//////////----------------------
  ...
  * E.g.:
  * <pre>
  * M/d/yy h:mm:ss a -> MM/dd/yyyy HH:mm:ss
  * d.M.yy HH:mm: nach dd.MM.yyyy HH:mm:ss:
  * yy'年'M'月'd'日' ahh:mm:ss -> yyyy'年'MM'月'dd'日' ahh:mm:ss
  * </pre>
  */
  public String getNormalizedDatePattern(String pattern, boolean force24h) {
    ...
    // (Java 1.4.2, Linux) mk / mk_MK: dd.MM.yyyy HH:mm: -> dd.MM.yyyy HH:mm:ss:
    // (Java 1.6.0, Linux) zh_SG:      dd/MM/yyyy a hh:mm -> dd/MM/yyyy a hh:mm:ss
    ...
    if (pattern.indexOf("s") < 0) {
      ...
      if (idx >= 0) { // If not, it not even has a minutes part???
        pattern = pattern.substring(0,idx+2) + // Pattern before "mm"
            pattern.substring(idx-1,idx) + // Same separator as before mm, assuming the pattern is hh:mm or similar
            "ss" + // seconds
            pattern.substring(idx+2); // Rest of the pattern
      }
    }
