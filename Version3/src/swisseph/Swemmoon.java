 ...
 */
  ...
  void swi_mean_lunar_elements(double tjd,
                               ...
                               double dnode[] /* double used as output parameter */,
                               double peri[] /* double used as output parameter */,
                               double dperi[] /* double used as output parameter */) {
    T = (tjd - SwephData.J2000) / 36525.0;
    T2 = T*T;
    mean_elements();
    node[0] = sl.swe_degnorm((SWELP - NF) * SwephData.STR * SwissData.RADTODEG);
    peri[0] = sl.swe_degnorm((SWELP - MP) * SwephData.STR * SwissData.RADTODEG);
    T -= 1.0 / 36525;
    mean_elements();
    dnode[0] = sl.swe_degnorm(node[0] - (SWELP-NF) * SwephData.STR * SwissData.RADTODEG);
    dnode[0] -= 360;
    dperi[0] = sl.swe_degnorm(peri[0] - (SWELP-MP) * SwephData.STR * SwissData.RADTODEG);
  }
